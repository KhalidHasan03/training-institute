@php
    ...
    $currentBatchId = (int) (request()->query('batch') ?: session('trainer_batch_id'));
    $currentBatch = $batches->firstWhere('id', $currentBatchId) ?? $batches->first();
@endphp
    ...
    <div class="flex items-center gap-2" x-data="{ open: false }">
        ...
        <div class="relative">
            <button @click="open = !open" class="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 shadow-sm transition-colors hover:border-brand-300 hover:text-brand-700">
                ...
                <span>{{ $currentBatch?->name ?? 'Select batch' }}</span>
            ...
            <div x-show="open" x-cloak @click.outside="open = false" class="absolute right-0 z-50 mt-2 w-72 overflow-hidden rounded-2xl border border-slate-200 bg-white p-1.5 shadow-lift">
                @foreach ($batches as $batch)
                    <a href="{{ url()->current() }}?batch={{ $batch->id }}"
                       class="flex items-center justify-between gap-3 rounded-xl px-3 py-2.5 text-sm transition-colors {{ $batch->id === $currentBatch?->id ? 'bg-brand-50 text-brand-800' : 'text-slate-700 hover:bg-slate-50' }}">
                        <span class="min-w-0">
                            <span class="block truncate font-semibold">{{ $batch->course->title }}</span>
                            <span class="block text-xs text-slate-400">Batch {{ $batch->name }}</span>
                        </span>
