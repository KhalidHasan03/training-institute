<div>
    ...
    @if (session()->has('profile-saved'))
        <div class="mt-6 flex items-center gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-4">
            ...
            <p class="text-sm font-medium text-emerald-800">{{ session('profile-saved') }}</p>
        </div>
    @endif

    @if (session()->has('password-saved'))
        <div class="mt-6 flex items-center gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-4">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor" class="h-5 w-5 text-emerald-600"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" /></svg>
            <p class="text-sm font-medium text-emerald-800">{{ session('password-saved') }}</p>
        </div>
    @endif
    ...
    <div class="mt-6 grid gap-6 lg:grid-cols-3">
        <div class="space-y-6 lg:col-span-2">
            <form wire:submit="save" class="card p-6">
                ...
                <div class="mt-6 flex items-center gap-4">
                    <div class="relative h-16 w-16 shrink-0 overflow-hidden rounded-2xl bg-brand-100">
                        ...
                        @elseif ($this->student?->photo)
                            <img src="{{ Storage::url($this->student->photo) }}" alt="{{ $name }}" class="h-full w-full object-cover">
                        @else
                ...
                <div class="mt-6 grid gap-4 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input id="date_of_birth" type="date" wire:model="date_of_birth" class="input mt-1.5">
                        @error('date_of_birth')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <input id="address" type="text" wire:model="address" class="input mt-1.5">
                        @error('address')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="card p-6">
                <h3 class="font-display text-base font-bold text-slate-900">Account</h3>
                <p class="mt-1 text-sm text-slate-500">Your login details.</p>
                <div class="mt-6 grid gap-4 sm:grid-cols-2">
                    <div>
                    ...
                    <div>
                        <label for="student_id" class="text-sm font-medium text-slate-700">Student ID</label>
                        <input id="student_id" type="text" value="{{ $this->student?->student_id ?? '—' }}" disabled class="input mt-1.5 cursor-not-allowed bg-slate-50">
                    </div>
                </div>
            </div>
        </div>
