...
<head>
    ...
    <title>{{ $title ?? 'Student Portal' }} — {{ config('app.name') }}</title>
...
<body class="min-h-screen bg-slate-100 font-sans" x-data="{ sidebarOpen: false }">
    ...
    <div class="min-h-screen">
        ...
        {{-- Sidebar (fixed/subtle: never scrolls away, nav scrolls independently) --}}
        <aside class="fixed inset-y-0 left-0 z-50 flex w-64 -translate-x-full transform flex-col bg-navy-950 shadow-2xl transition-transform duration-200 lg:translate-x-0"
                ...
                <div class="relative flex w-full items-center justify-between">
                    <a href="{{ route('student.dashboard') }}" class="flex items-center gap-2">
                        ...
                        <span class="font-display text-sm font-extrabold text-white">
                            ...
                            <span class="text-gradient">{{ strstr(config('app.name'), ' ') ?: 'Portal' }}</span>
                        </span>
                    </a>
            ...
            {{-- Nav (scrollable so it never hides under the footer bar) --}}
            <nav class="flex-1 overflow-y-auto px-3 py-5">
                <p class="px-3 pb-2 text-[10px] font-semibold uppercase tracking-widest text-slate-500">Navigation</p>
                <div class="space-y-1">
                    <x-student.nav-link href="{{ route('student.dashboard') }}" :active="request()->routeIs('student.dashboard')" icon="home">Dashboard</x-student.nav-link>
                    <x-student.nav-link href="{{ route('student.my-course') }}" :active="request()->routeIs('student.my-course')" icon="book">My Course</x-student.nav-link>
                    <x-student.nav-link href="{{ route('student.schedule') }}" :active="request()->routeIs('student.schedule')" icon="calendar">Schedule</x-student.nav-link>
                    <x-student.nav-link href="{{ route('student.attendance') }}" :active="request()->routeIs('student.attendance')" icon="clipboard">Attendance</x-student.nav-link>
                    <x-student.nav-link href="{{ route('student.materials') }}" :active="request()->routeIs('student.materials')" icon="folder">Materials</x-student.nav-link>
                    <x-student.nav-link href="{{ route('student.assignments') }}" :active="request()->routeIs('student.assignments')" icon="pencil">Assignments</x-student.nav-link>
                    <x-student.nav-link href="{{ route('student.results') }}" :active="request()->routeIs('student.results')" icon="chart">Results</x-student.nav-link>
                    <x-student.nav-link href="{{ route('student.payments') }}" :active="request()->routeIs('student.payments')" icon="wallet">Payments</x-student.nav-link>
                    <x-student.nav-link href="{{ route('student.certificate') }}" :active="request()->routeIs('student.certificate')" icon="award">Certificate</x-student.nav-link>
                    <x-student.nav-link href="{{ route('student.profile') }}" :active="request()->routeIs('student.profile')" icon="user">Profile</x-student.nav-link>
                </div>
            </nav>
        ...
        <div class="flex min-h-screen flex-col lg:pl-64">
            ...
            <header class="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-slate-200 bg-white/90 px-4 backdrop-blur sm:px-6">
                <div class="flex items-center gap-3">
                    ...
                    <h2 class="hidden font-display text-base font-bold text-slate-900 sm:block">Student Portal</h2>
                </div>

                <div class="flex items-center gap-3">
                    @include('partials.student.batch-switcher')
                </div>
                ...
                <div class="relative" x-data="{ userMenu: false }">
                    <button @click="userMenu = !userMenu" class="flex items-center gap-3 rounded-xl p-1.5 pr-3 hover:bg-slate-100">
                        <span class="grid h-9 w-9 place-items-center rounded-full bg-gradient-to-br from-brand-500 to-accent-600 text-sm font-bold text-white shadow-glow">
                            {{ strtoupper(substr(auth()->user()->student?->name ?? auth()->user()->name, 0, 1)) }}
                        </span>
                        <span class="hidden text-left sm:block">
                            <span class="block max-w-[10rem] truncate text-sm font-semibold text-slate-800">{{ auth()->user()->student?->name ?? auth()->user()->name }}</span>
                            <span class="block text-xs text-slate-400">{{ auth()->user()->student?->student_id }}</span>
                        </span>
                    </button>
                    <div x-show="userMenu" x-cloak @click.outside="userMenu = false" class="absolute right-0 mt-2 w-56 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-lift">
                        <a href="{{ route('student.profile') }}" class="flex items-center gap-2 px-4 py-3 text-sm text-slate-700 hover:bg-slate-50">My Profile</a>
                        <a href="{{ route('public.courses') }}" class="flex items-center gap-2 px-4 py-3 text-sm text-slate-700 hover:bg-slate-50">Browse Courses</a>
                        <a href="{{ route('public.certificates.verify') }}" class="flex items-center gap-2 px-4 py-3 text-sm text-slate-700 hover:bg-slate-50">Verify Certificate</a>
